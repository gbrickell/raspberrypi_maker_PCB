...
#
# command: python3 /home/pi/RPi_maker_kit5/sensors/object_detection/PIR_detection/PIR_detect01.py
#
...
trigger_interval = 2    # minimum interval in seconds between detections to allow the electronics to reset
...
print (now + " - program running : using a SR501, SR312 or SR602 warm body detection to print an output - type CTRL-C to stop")
try:    # this loop is not strictly necessary but it does allow the script to be easily stopped with CTRL-C

    #+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    # wait a short period for the SR501, SR312 or SR602 module electronic to settle 
    #+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    print ("Waiting for SR602 electronics to initially settle ...")
    ...
    while True:  # this is the loop that checks if the SR602 is triggered

        #+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
        # check for movement detection
        #+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
        # Read SR602 state from the input GPIO pin
        Current_State = GPIO.input(sense_pin)
        #print (" Current State = " + str(Current_State))  # debug output usually commented out

        if Current_State == 1:
            #SR602 is triggered 
            print (" SR501, SR312 or SR602 detected warm body at: " + time.strftime('%a %d %b %Y %H:%M:%S %Z'))
            print ("**********************************************************************")
            time.sleep(trigger_interval)
            print (" waited " + str(trigger_interval) + " seconds to let the electronics reset")
            print (" Looking again for a warm body to be detected")
            Current_State  = 0
