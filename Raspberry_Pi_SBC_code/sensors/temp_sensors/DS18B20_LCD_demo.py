...
# Date   : 210422
# command to run:  python3 /home/pi/RPi_maker_kit5/sensors/temp_sensors/DS18B20_LCD_demo.py
...
if os.path.isfile(temp_sensor):
    ...
    try:    # this loop is not strictly necessary but it does allow the script to be safely stopped with CTRL-C
        while True:
                ...
                mylcd.lcd_display_string("{:.3f}".format(tempc) + " degC", 1, 2)   # display at row 1 column 2
                mylcd.lcd_display_string("{:.3f}".format(tempf) + " degF", 2, 2)   # display at row 2 column 2
                time.sleep(1)
        ...
        print ("  ")
        mylcd.lcd_clear()       # clear LCD screen
        mylcd.backlight(0)      # backlight off
else:
    print ("***************************************************************")
