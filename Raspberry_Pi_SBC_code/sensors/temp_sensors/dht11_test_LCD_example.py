...
# command to run: python3 /home/pi/RPi_maker_kit5/sensors/temp_sensors/dht11_test_LCD_example.py
...
try:
    ...
    while True:
        ...
        if result.is_valid():                            # check if the data collected is valid True or False
            ...
            print ("update LCD display")
            mylcd.lcd_clear()  # clear LCD screen
            mylcd.lcd_display_string("temperature: %dC" % result.temperature, 1, 0)   # display at row 1 column 0
            mylcd.lcd_display_string("   humidity: %d%%" % result.humidity, 2, 0)   # display at row 2 column 0
        ...
        time.sleep(10)   # wait 10 second before cycling back to take another set of readings: sensor is only ready every 2 seconds
