# RPi PCB version of LED_button_buzzer.py that lights a RED LED and sounds a passive buzzer for 'duration' (3) secs when a button is pressed
...
LED_plus_pin = 16  # this is the GPIO pin that the RED LED positive leg (via the resistor) is connected to

buzzer_pin = 12    # this is the GPIO pin that the positive pin of the buzzer s connected to

button_pin = 7     # this is the GPIO pin that one side of the (top: button1) tactile switch is connected to
...
print (" ")
print ("program running: press button 1 (top) to light the LED or CTRL-C to stop ")
print (" ")
...
try:    # this loop is not strictly necessary but it does allow the script to be easily stopped with CTRL-C
    while True:  # this is the loop that checks if the button is pressed and switches the LED on if it is
        ...
        print(" ")
        print("button 1 pressed and LED + buzzer switched on")
        print(" ")
