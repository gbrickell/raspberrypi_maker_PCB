# RPi kits PCB version of LED_red_amber_green_flash.py that alternates on/off for three LEDs (red, amber & green)

# command to run this script:  python3 /home/pi/RPi_maker_kit5/electronic_basics/LED_red_amber_green_flash.py
...
red_positive_pin = 16    # this is the GPIO pin that the red LED positive leg (via the resistor) is connected to

amber_positive_pin = 20  # this is the GPIO pin that the amber LED positive leg (via the resistor) is connected to

green_positive_pin = 21  # this is the GPIO pin that the green LED positive leg (via the resistor) is connected to
