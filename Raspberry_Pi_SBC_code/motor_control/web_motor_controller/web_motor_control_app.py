...
# RPi kits PCB version of servo_stepper_control_app.py
# Servo & Stepper Motor Kit Flask controller: python code as part of a Flask based web server system
# 
...
def stepmotor(direction, sequences, position):
    ...
    for seqloop in range (0, sequences):
...
def start():
    ...
    return render_template('select_motor_mode.html', **template_data)


#########################################################################################################
# this route defines the actions selected when the servo & stepper motor system is in the selection mode
#########################################################################################################
@servo_stepper_app01.route("/<choice_mode>")  # run the code below this function when URL /<choice_mode> is accessed from select_motor_mode.html where choice_mode is a variable
...
if __name__ == "__main__":
    # using the 'production' flask server with sudo so OK to use port 80
    servo_stepper_app01.run(host='0.0.0.0', port=80, debug=False, threaded=True)   # 0.0.0.0 means any device on the network can access the web app
    #servo_stepper_app01.run(host='0.0.0.0', port=80, debug=True)   # option to run without 'threaded' set to True and debug 'on'
