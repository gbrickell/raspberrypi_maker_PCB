...
# RPi PCB image streaming Flash app
# Image taking USB camera Flask controller: python code as part of a Flask based web server system
#  - this version of the app is for use by user with root privileges (i.e. sudo) and uses the 80 port
#    AND it needs for a ram drive to have been previously set up by root
# 
#  - this version uses the USB camera class that annotates the image

# command to run: sudo python3 ./RPi_maker_PCB5/image_taking/image_taking_controller/image_streaming_app_root_annotate.py
...
if __name__ == "__main__":
    make_image_app01.run(host='0.0.0.0', port=80, debug=False, threaded=True) # 0.0.0.0 means any device on the network can access the web app
    # uses port 80 so you must run the app as root (ie use sudo) which is necessary if the 'production' port 80 is used

    # debug version
    #make_image_app01.run(host='0.0.0.0', port=80, debug=True)   # 0.0.0.0 means any device on the network can access the web app
